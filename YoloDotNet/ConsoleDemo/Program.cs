...
for (int i = 0; i < files.Length; i++)
{
    ...
    runDemoAction(ModelType.ObjectDetection, ModelVersion.V8, ImageType.Street, file_name,false, false);
    runDemoAction(ModelType.ObjectDetection, ModelVersion.Vx, ImageType.Street, file_name, false, false);
    //runDemoAction(ModelType.ObjectDetection, ModelVersion.V9, ImageType.Street, file_name, false, false);
    //runDemoAction(ModelType.ObjectDetection, ModelVersion.V10, ImageType.Street, file_name,false, false);
    //runDemoAction(ModelType.ObjectDetection, ModelVersion.V11, ImageType.Street, file_name,false, false);
}
...
static void RunDemoX(ModelType modelType, ModelVersion modelVersion, ImageType imageType,string image_file, bool cuda = false, bool primeGpu = false)
{
    string SaveDir;
    ...
    SaveDir = DemoSettings.OUTPUT_FOLDER + "\\"+$"{modelType}_{modelVersion}"+"\\"+Path.GetFileName(Path.GetDirectoryName(image_file));
    if(Directory.Exists(SaveDir) == false)
        Directory.CreateDirectory(SaveDir);
    resultImage.Save(SaveDir+"\\"+Path.GetFileName(image_file), SKEncodedImageFormat.Jpeg);
}
static void RunDemo_backup(ModelType modelType, ModelVersion modelVersion, ImageType imageType, bool cuda = false, bool primeGpu = false)
{
    var modelPath = modelVersion switch
    {
        ModelVersion.V8 => SharedConfig.GetTestModelV8(modelType),
    ...
    var imagePath = @"F:\Recive\group1\000001.jpg";
